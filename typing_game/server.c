...
void *client_handler(void *arg) {
    ...
    if (current_room_id != -1) {
        ...
        Room *current_room = find_room(current_room_id);
        if (current_room != NULL) {
            remove_user_from_room(current_room, find_user(socket_fd));
...
// 임시로 gameover확인용 코드 수정요망 *********************************

void send_gameover_to_all_clients(const char *message) {
    pthread_mutex_lock(&user_mutex);
    User *current_user = user_head;

    while (current_user != NULL) {
        send_message(current_user->socket_fd, message);
        current_user = current_user->next;
    }
