...
#define INPUT_BUFFER_SIZE 1024 // Limit input buffer size to prevent overflow
...
void *recv_handler(void *arg) {
    ...
    while ((bytes_read = recv(sock, buffer, sizeof(buffer) - 1, 0)) > 0 && running) {
        ...
        if (strncmp(buffer, "CHAT ", 5) == 0) {
            ...
            wprintw(chat_win, "어서오세요~~ 타자의 짱을 가려라 타짱이에요, %s님!\n", user_name);
...
void game_handle_input() {
    ...
    if (c == '\n') {
        ...
        if (game_enter_position > 0) {
            game_enter_position--;
            game_typingText[game_enter_position] = '\0';
            mvprintw(LINES - 1, 13 + game_enter_position, " ");
            move(LINES - 1, 13 + game_enter_position);
        }
    } else if (isprint(c) && game_enter_position < (int)(sizeof(game_typingText) - 1)) {
        game_typingText[game_enter_position++] = c;
        game_typingText[game_enter_position] = '\0';
        mvprintw(LINES - 1, 13, "%s", game_typingText);
        move(LINES - 1, 13 + game_enter_position);
    }
