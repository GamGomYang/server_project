...
void start_game() {
	...
	addstr("Level");
	move(1, 78);
	snprintf(level_bar,3, "%02d", level);
	addstr(level_bar);
	move(0, 15);
	addstr("ADDING_MODE");
	...
	move(1, 1);
	addstr("score : ");
	snprintf(score_bar,4, "%03d", score);
	addstr(score_bar);

	// hp print
	move(0, 1);
	addstr("life  : ");
	snprintf(hp_bar,4, "%3d", hp);
	addstr(hp_bar);
	refresh();
...
void * game_Board(void *m)
{
	...
	while (hp > 0) {
		...
		for (int i = 2; i < 20; i++) blankdraw(i);//screen clear
		...
		// 문자열 전부 출력
		while (temp) {
			int touch_cute = cute_pos_comp(temp->row, temp->col, strlen(temp->str));
			...
			if (touch_cute) {//건드렸다면 라이프 감소 후 문자열 공백
				strcpy(temp->str, "");
				hp--;
				move(0, 1);
				addstr("life  : ");
...
void cute() {
	if (!cute_mode) {
		...
		move(19, 60 - move_cute);
		addstr(cute2_4);
		move(18, 63 - move_cute);
		addstr(cute2_3);
		move(17, 63 - move_cute);
		addstr(cute2_2);
		move(16, 63 - move_cute);
		addstr(cute2_1);
...
int cute_pos_comp(int row, int col, int len) {
	...
	if (cute_mode == 1) {
		if (row >= 16) {// 쪼그린 상태
			if (row >= 19) {											// 아랫부분
				for (int i = 0; i < len; i++) {
					if ((col + i >= cute_col_1) && (col + i < (cute_col_1 + 11))) {
						return 1;
					}
				}
			}
