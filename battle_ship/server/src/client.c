...
void inputLoop(Cell own_grid[GRID_SIZE][GRID_SIZE], Cell opponent_grid[GRID_SIZE][GRID_SIZE]) {
    Cursor cursor = {0, 0}; // 초기 커서 위치 (상대방 그리드의 좌측 상단)
...
int main(int argc, char **argv) {
    ...
    start_color();
    /* 추가적인 색상 정의 */
    init_pair(1, COLOR_BLUE, COLOR_BLACK);    // 배 (Ship)
